...
func doRemoval(m [][]bool) (count int, nextRound [][]bool) {
	count = 0
	nextRound = make([][]bool, len(m))

	for i := 0; i < len(m); i++ {
		nextRound[i] = make([]bool, len(m[i]))
		for j := 0; j < len(m[i]); j++ {
			if !m[i][j] {
				//fmt.Print(".")
				continue
