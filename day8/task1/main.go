...
func main() {
	f, err := os.Open("day8/task1/input.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer f.Close()
	...
	for k := range circuitSets {
		c := *k
		for _, p := range c {
			fmt.Printf("(%v,%v,%v) ", p.X, p.Y, p.Z)
		}
		fmt.Println()

		lArr = append(lArr, len(c))
	}

	slices.SortFunc(lArr, func(a, b int) int { return -cmp.Compare(a, b) })
	fmt.Println(lArr[0], lArr[1], lArr[2])
	fmt.Println(lArr[0] * lArr[1] * lArr[2])
}
